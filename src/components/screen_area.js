...
const ScreenArea = () => (
        <MessagesContext.Consumer>{(messagesContext) => (
            <ColorContext.Consumer>{(colorContext) => {
                ...
                const {singleMes, handleSingleMes, clickHandler,canvasDisplaying} = messagesContext;
                return (
                    ...
                    <div className='input-write-and-canvas-area' style={{color: fontColor}}>
                        <input className='content-editable-input' value={singleMes} style={{color: fontColor}} onChange={(e) => handleSingleMes(e.currentTarget.value)}/>
